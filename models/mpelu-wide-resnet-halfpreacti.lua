...
local function createModel(opt)
   ...
   local function wide_basic(nInputPlane, nOutputPlane, stride)
      ...
      for i,v in ipairs(conv_params) do
         if i == 1 then  -- in which case, v == {3,3,stride,stride,1,1}
            ...
            -- module:add(SBatchNorm(nInputPlane ,false))
            module:add(MPELU(alpha, beta, 5, 5, 5, 5, nInputPlane))
            ...
            convs:add(SBatchNorm(nBottleneckPlane, false)):add(MPELU(alpha, beta, 5, 5, 5, 5, nBottleneckPlane))
      ...
      local shortcut = nInputPlane == nOutputPlane and
         nn.Identity() or
         nn.Sequential():add(Convolution(nInputPlane,nOutputPlane,1,1,stride,stride,0,0))
                        :add(SBatchNorm(nOutputPlane, false))
   ...
   do
      ...
      model:add(MPELU(alpha, beta, 5, 5, 5, 5, nStages[4]))
